...
if [ ! -e ~/.running ] || [ $[`date +%s` - `date +%s -r ~/.running`] -gt 600 ]; then
    touch ~/.running
    find . -name "dist" -prune  -o  -name ".git" -prune  -o -name "objs" -prune  -o -name "clientsv4" -prune  \
        -o -type l -prune \
    ...
    cscope -kRb 
    ctags -R --fields=+lStn --extra=+f --tag-relative=yes --exclude=objs --exclude=dist --links=no -f tags.tmp 
    mv -f tags.tmp tags
